      ...
      >
        <template #bodyCell="{ column, record, index }">
            ...
            <template v-if="column.key === 'amount_total'">
              <div class="text-center">{{ record.amount_total }}</div>
            </template>
...
import { getFirstDepositListAliianceServices } from '~/services/allianceServices';
...
const dynamicColumns = computed(() => {
  ...
  { title: `ทั้งหมด ${allRecord.value} รายการ`, children: [
    ...
    { title: 'จำนวนเงิน', width:80, dataIndex: 'amount_total', key: 'amount_total' },
...
const getDataAlliance = async() => {
  ...
  const data = await getFirstDepositListAliianceServices(formData,ref_value,token,permission);
